describe("Redux", () => {
	...
	describe('state', () => {
		...
		it('requires "com.drpicox.angularjs-redux" module and initial store.getState() is {}', () => {
			...
			expect(new Object).toEqual(state);
		});
	...
	describe('reducer', () => {
		it('you can configure a new reducer with initial value', () => {
			...
			expect(Object({constantThree:3}).toEqual(state);
		});
		...
		it('you can configure multiple reducers', () => {
			...
			expect(Object({name:'Hoboken',animals:[ ]})).toEqual(state);
		});
		...
		it('reducers can respond to dispatched actions', () => {
			...
			expect(Object({count:1}).toEqual(state);		
		});
		...
		it('reducers never modify the state', () => {
			...
			expect(Object({animals:['savio']})).toEqual(state);		
		});
		...
		it('reducers can use auxiliary reducers', () => {
			...
			expect(Object({ animals: [Object({ name: 'savio', present: false }), 
				   	Object({ name: 'lulu', present: true })]})).toEqual(state);		
		});
	});
	...
	describe('dispatcher', () => {
		...
		it('you can create reusable action dispatchers', () => {
			...
			expect(Object({type:'INCREMENT'})).toEqual(testbed.getLastDispatchedAction());
		});
		...
		it('action constructor for dispatch can have parameters', () => {
			...
			expect(Object({type:'ADD_ANIMAL',name:'savio'})).toEqual(testbed.getLastDispatchedAction());
		});
		...
		it('dispatched actions triggers reducers', () => {
			...
			dispatcher.increment();
			expect(Object({count:1})).toEqual(testbed.getState())
		});
